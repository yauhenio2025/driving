// Auto-generated from Chinese Road Traffic Safety Law (2003)
// Source: Wikisource
...
// Maps each question category to relevant law article texts
const chapterMap = {};
for (const ch of trafficLaw.chapters) {
  chapterMap[ch.number] = ch.articles;
}
...
export const categoryToChapters = {};
for (const [category, chNums] of Object.entries(categoryChapterMap)) {
  categoryToChapters[category] = [];
  for (const n of chNums) {
    const arts = chapterMap[n] || [];
    for (const a of arts) {
      categoryToChapters[category].push(`Article ${a.number}: ${a.text}`);
    }
  }
}
